...
export async function getChildIds(parentId: string): Promise<string[]> {
  const response = await fetch(`${PARENT_SERVICE_URL}/children?parentId=${encodeURIComponent(parentId)}`);
  if (!response.ok) {
    console.warn(`[parentServiceClient] GET /children?parentId=${parentId} returned ${response.status}`);
    return [];
  }
