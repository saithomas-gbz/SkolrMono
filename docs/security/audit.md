...
| **A09** | Security Logging and Monitoring Failures | ✅ Partiel | Logs de sécurité **structurés** sur les points sensibles (**R9**) ; **Sentry** (backend + frontend) pour les exceptions ; **Prometheus + Grafana dans la stack de production** avec métriques applicatives (latence, trafic et taux d'erreur par route), métriques hôte/base, et **7 règles d'alerte** couvrant l'indisponibilité, le taux de 5xx, la latence, l'espace disque et le retard des sauvegardes (#218, voir `docs/ops/supervision.md`). Ce qui reste partiel : les alertes s'évaluent et s'affichent mais **ne sont poussées vers personne** (pas encore d'Alertmanager) ; alerting métier sur `login.failed`/`reuse_detected` toujours en perspective (#168) |
...
**Bilan** : 7 catégories pleinement couvertes, 2 partielles (**A08**, signature d'images ; **A09**, acheminement des alertes), 1 non applicable justifiée (**A10**). Les 10 catégories sont donc adressées ou explicitement écartées. **A06** est passée de partielle à couverte avec la livraison de Dependabot et CodeQL (#196). **A09** reste partielle mais change de nature : la supervision applicative en production existe désormais (#218), il ne manque plus que la notification des alertes.
